...
async function copyTemplateCollections(targetAdminId, eventId) {
    try {
        ...
        // テンプレートの実際の構造に合わせて調整
        // 2層構造のテンプレートから3層構造のターゲットへコピー
        const sourceCollectionName = "admin_collections/testtemplate";
        const targetCollectionName = `admin_collections/${targetAdminId}/${eventId}`;

        console.log(
            `コピー元: ${sourceCollectionName} → コピー先: ${targetCollectionName}`
        );

        // testtemplateからのコピーを有効化
        console.log("🚀 testtemplateからのコピーを開始...");
        try {
            ...
            console.log("✅ testtemplateからのコピー完了");
        } catch (templateError) {
        ...
        console.log("💡 高速化のヒント: 大量データの場合、Firestoreインデックス作成により表示速度が大幅に向上します");
        console.log("📊 管理画面でエラーが表示された場合は、提示されるリンクからインデックスを作成してください");
    } catch (error) {
...
async function createInitialCollections(targetCollectionName) {
    ...
    try {
        ...
        console.log(`[DEBUG] doc()セグメント確認: "${adminCollections}", "${adminId}", "${eventId}", "sample-item-001"`);
        console.log(`[DEBUG] セグメント数確認: ${[adminCollections, adminId, eventId, "sample-item-001"].length}個`);
        ...
        let scanItemsRef;
        try {
            scanItemsRef = doc(db, adminCollections, adminId, eventId, "_scanItems_init");
...
async function copySubCollections(
    ...
    try {
        ...
        for (const subColName of subCollectionNames) {
            console.log(`コピー中: ${sourceCollection}/${subColName} → ${targetCollection} (collection_type="${subColName}")`);
            ...
            try {
                ...
                console.log(`${sourceCollectionPath} から ${sourceSnapshot.docs.length}件のドキュメントを取得`);
                ...
                sourceSnapshot.docs.forEach((docSnap) => {
                    ...
                    // 3層構造のターゲット参照を作成
                    const targetRef = doc(db, adminCollections, targetAdminId, targetEventId, docSnap.id);
                    console.log(`[DEBUG] ターゲットドキュメント参照: ${adminCollections}/${targetAdminId}/${targetEventId}/${docSnap.id}`);
                ...
                if (batchCount > 0) {
                    await batch.commit();
                    console.log(`${subColName} コピー完了: ${sourceSnapshot.docs.length}件 → 3層構造`);
                }

            } catch (subError) {
                console.error(`サブコレクション ${subColName} のコピーエラー:`, subError);
