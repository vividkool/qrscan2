...
document.addEventListener("DOMContentLoaded", function () {
    ...
    onAuthStateChanged(auth, (user) => {
        if (user) {
            // 認証済みならadmin.htmlへリダイレクト
            window.location.href = "./admin.html";
        }
        // 未認証なら何もしない（index.htmlに留まる）
    });
});
...
function showAdminAuthInterface() {
    ...
    <button type="button" onclick="window.location.href='https://qrscan2-99ffd.web.app/login.html'" class="btn-link">ログイン画面へ</button>
        <!-- デモ用情報表示 -->
                ...
                <form onsubmit="handleAdminLogin(event)">
                    <div class="form-group">
                        ...
                        <input type="text" id="loginAdminId" name="adminId" required placeholder="ADMIN ID" value="${localStorage.getItem("currentAdmin")
