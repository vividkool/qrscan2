...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (qrUid) {
    ...
    if (qrUid.startsWith("demo_")) {
      ...
      if (
        roleFromUid === "user" &&
        window.location.pathname.includes("maker.html")
      ) {
...
async function performFirebaseAuth() {
  ...
  if (userData && userData.role) {
    ...
    if (userData.role === "user" && currentPage === "maker.html") {
      ...
      console.log(
        `${userData.role}ユーザーのためmaker.htmlから適切なページにリダイレクト`
      );
      // デフォルトで適切なページにリダイレクト
  ...
  if (makerItemsElement && user.user_name) {
    try {
      makerItemsElement.innerHTML =
        '<div class="loading">関連アイテムを読み込み中...</div>';
      ...
      makerItemsElement.innerHTML =
        '<div class="error">関連アイテムの表示でエラーが発生しました</div>';
    }
  }
}
